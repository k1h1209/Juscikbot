...
router.get("/:id", async (req, res) => {
    try {
        ...
        if (!stock) return res.status(404).json({ error: "존재하지 않는 주식입니다." });
        res.json({ ok: true, stock: publicStock(stock) });
    } catch (error) {
...
router.get("/:id/history", async (req, res) => {
    try {
        ...
        const history = await getHistory(req.params.id, range);
        res.json({ ok: true, stockId: req.params.id, range, history });
    } catch (error) {
        console.error("STOCK HISTORY ERROR:", error);
        res.status(500).json({ error: "주가 기록을 불러오지 못했습니다." });
    }
});
...
router.post("/buy", auth, async (req, res) => {
    ...
    try {
        ...
        if (!stockId || !Number.isInteger(qty) || qty <= 0) {
            return res.status(400).json({ error: "올바른 종목과 수량을 입력하세요." });
        }
        ...
        if (!stockResult.rows.length) {
            ...
            return res.status(404).json({ error: "존재하지 않는 주식입니다." });
        }

        const stock = stockResult.rows[0];
        const price = Number(stock.price);
        const total = price * qty;
        const available = Number(stock.available_shares || 0);

        if (qty > available) {
            await client.query("ROLLBACK");
            return res.status(400).json({
                error: `공유 거래 가능 수량이 부족합니다. 현재 ${available.toLocaleString()}주만 남았습니다.`
            });
        }

        const userResult = await client.query(`
            SELECT cash,holdings,transactions
            FROM users
            WHERE id=$1
            FOR UPDATE
        `, [req.user.id]);
        ...
        await client.query(`
            ...
            WHERE id=$4
        `, [cash - total, JSON.stringify(holdings), JSON.stringify(transactions), req.user.id]);

        await client.query(`
            UPDATE stocks
            SET volume=volume+$1,
                available_shares=available_shares-$1
            WHERE id=$2
        `, [qty, stockId]);
...
router.post("/sell", auth, async (req, res) => {
    ...
    try {
        ...
        const cash = Number(user.cash);
        const holdings = user.holdings || {};
        const currentQuantity = Number(holdings[stockId] || 0);

        if (currentQuantity < qty) {
            await client.query("ROLLBACK");
            return res.status(400).json({ error: "보유한 주식보다 많이 팔 수 없습니다." });
        }

        const transactions = user.transactions || [];
        const newQuantity = currentQuantity - qty;
        if (newQuantity === 0) delete holdings[stockId];
        else holdings[stockId] = newQuantity;

        transactions.push({ type: "sell", stockId, stockName: stock.name, quantity: qty, price, total, time: Date.now() });

        const maxShares = Number(stock.max_shares || 0);
        const newAvailable = Math.min(maxShares, Number(stock.available_shares || 0) + qty);
