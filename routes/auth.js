const express = require("express");
const crypto = require("crypto");
const { pool } = require("../services/market");

const router = express.Router();

const FALLBACK_STARTING_CASH = 10000;

function hashPassword(password, salt) {
  return crypto.scryptSync(password, salt, 64).toString("hex");
}

function createToken() {
  return crypto.randomBytes(32).toString("hex");
}

function safeUser(user) {
  return {
    id: user.id,
    playerNumber: user.player_number ? Number(user.player_number) : null,
    username: user.username,
    nickname: user.nickname,
    cash: Number(user.cash),
    holdings: user.holdings || {}
  };
}

function isUserBanned(user) {
  if (user.banned_until === null || user.banned_until === undefined) return false;
  return Number(user.banned_until) > Date.now();
}

async function getStartingCash() {
  const { rows } = await pool.query(
    "SELECT starting_cash FROM game_settings WHERE id=1 LIMIT 1"
  );

  if (!rows.length) return FALLBACK_STARTING_CASH;

  const value = Number(rows[0].starting_cash);
  return Number.isFinite(value) && value >= 0
    ? value
    : FALLBACK_STARTING_CASH;
}

async function auth(req, res, next) {
  try {
    const header = req.headers.authorization || "";
    const token = header.startsWith("Bearer ") ? header.substring(7) : "";

    if (!token) {
      return res.status(401).json({ error: "로그인이 필요합니다." });
    }

    const result = await pool.query(
      `
        SELECT
          u.id,
          u.player_number,
          u.username,
          u.nickname,
          u.cash,
          u.holdings,
          u.transactions,
          u.banned_until,
          u.ban_reason
        FROM sessions s
        JOIN users u ON u.id=s.user_id
        WHERE s.token=$1
      `,
      [token]
    );

    if (!result.rows.length) {
      return res.status(401).json({ error: "로그인이 필요합니다." });
    }

    const user = result.rows[0];

    if (isUserBanned(user)) {
      await pool.query("DELETE FROM sessions WHERE token=$1", [token]);

      return res.status(403).json({
        error: "현재 계정이 이용 정지 상태입니다.",
        bannedUntil: Number(user.banned_until),
        reason: user.ban_reason || ""
      });
    }

    req.user = user;
    req.token = token;
    next();
  } catch (error) {
    console.error("AUTH ERROR:", error);
    res.status(500).json({ error: "인증 처리 중 오류가 발생했습니다." });
  }
}

router.post("/register", async (req, res) => {
  try {
    const { username, password, nickname } = req.body;

    if (!username || !password || !nickname) {
      return res.status(400).json({
        error: "아이디, 비밀번호, 닉네임을 모두 입력하세요."
      });
    }

    const cleanUsername = String(username).trim();
    const cleanNickname = String(nickname).trim();
    const cleanPassword = String(password);

    if (cleanUsername.length < 3) {
      return res.status(400).json({ error: "아이디는 3자 이상이어야 합니다." });
    }

    if (cleanPassword.length < 4) {
      return res.status(400).json({ error: "비밀번호는 4자 이상이어야 합니다." });
    }

    if (cleanNickname.length < 2) {
      return res.status(400).json({ error: "닉네임은 2자 이상이어야 합니다." });
    }

    const duplicate = await pool.query(
      `
        SELECT id
        FROM users
        WHERE LOWER(username)=LOWER($1) OR nickname=$2
        LIMIT 1
      `,
      [cleanUsername, cleanNickname]
    );

    if (duplicate.rows.length) {
      return res.status(409).json({
        error: "이미 사용 중인 아이디 또는 닉네임입니다."
      });
    }

    const id = crypto.randomUUID();
    const salt = crypto.randomBytes(16).toString("hex");
    const passwordHash = hashPassword(cleanPassword, salt);
    const token = createToken();
    const createdAt = Date.now();
    const startingCash = await getStartingCash();

    const result = await pool.query(
      `
        INSERT INTO users
        (id,username,nickname,salt,password_hash,cash,holdings,transactions,created_at)
        VALUES($1,$2,$3,$4,$5,$6,'{}'::jsonb,'[]'::jsonb,$7)
        RETURNING id,player_number,username,nickname,cash,holdings
      `,
      [
        id,
        cleanUsername,
        cleanNickname,
        salt,
        passwordHash,
        startingCash,
        createdAt
      ]
    );

    await pool.query(
      `
        INSERT INTO sessions(token,user_id,created_at)
        VALUES($1,$2,$3)
      `,
      [token, id, createdAt]
    );

    await pool.query(
      `
        INSERT INTO notifications(user_id,message,type,created_at)
        VALUES($1,$2,$3,$4)
      `,
      [id, "VSM 가입을 환영합니다!", "welcome", createdAt]
    );

    res.status(201).json({
      ok: true,
      token,
      user: safeUser(result.rows[0])
    });
  } catch (error) {
    console.error("REGISTER ERROR:", error);
    res.status(500).json({ error: "회원가입 중 오류가 발생했습니다." });
  }
});

router.post("/login", async (req, res) => {
  try {
    const { username, password } = req.body;

    if (!username || !password) {
      return res.status(400).json({ error: "아이디와 비밀번호를 입력하세요." });
    }

    const result = await pool.query(
      `
        SELECT *
        FROM users
        WHERE LOWER(username)=LOWER($1)
        LIMIT 1
      `,
      [String(username).trim()]
    );

    if (!result.rows.length) {
      return res.status(401).json({
        error: "아이디 또는 비밀번호가 올바르지 않습니다."
      });
    }

    const user = result.rows[0];

    if (isUserBanned(user)) {
      return res.status(403).json({
        error: "현재 계정이 이용 정지 상태입니다.",
        bannedUntil: Number(user.banned_until),
        reason: user.ban_reason || ""
      });
    }

    const passwordHash = hashPassword(String(password), user.salt);

    if (passwordHash !== user.password_hash) {
      return res.status(401).json({
        error: "아이디 또는 비밀번호가 올바르지 않습니다."
      });
    }

    const token = createToken();

    await pool.query(
      `
        INSERT INTO sessions(token,user_id,created_at)
        VALUES($1,$2,$3)
      `,
      [token, user.id, Date.now()]
    );

    res.json({ ok: true, token, user: safeUser(user) });
  } catch (error) {
    console.error("LOGIN ERROR:", error);
    res.status(500).json({ error: "로그인 중 오류가 발생했습니다." });
  }
});

router.get("/me", auth, async (req, res) => {
  res.json({ ok: true, user: safeUser(req.user) });
});

router.post("/logout", auth, async (req, res) => {
  try {
    await pool.query("DELETE FROM sessions WHERE token=$1", [req.token]);
    res.json({ ok: true });
  } catch (error) {
    console.error("LOGOUT ERROR:", error);
    res.status(500).json({ error: "로그아웃 중 오류가 발생했습니다." });
  }
});

module.exports = router;
module.exports.auth = auth;
