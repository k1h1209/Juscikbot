...
function randomFloorLockUntil() {
  const duration =
    FLOOR_LOCK_MIN_MS +
    Math.floor(Math.random() * (FLOOR_LOCK_MAX_MS - FLOOR_LOCK_MIN_MS + 1));
...
async function updateStockMarket(stock) {
  ...
  // 커트라인에 도달한 동안에는 주가가 올라가지 않고 그대로 유지됩니다.
  ...
  const { rows } = await pool.query(
    `SELECT direction,until_time,strength FROM market_controls WHERE stock_id=$1`,
    [stock.id]
  );

  const nextPrice = calculateNextPrice(stock, rows[0] || null);

  // 다음 변동이 커트라인 아래로 내려가면 커트라인에서 멈추고
  // 5~15분 동안 상승하지 않도록 잠급니다.
