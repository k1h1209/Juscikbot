...
async function initializeDatabase() {
  ...
  try {
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS users (
      id TEXT PRIMARY KEY, player_number SERIAL UNIQUE, username TEXT UNIQUE NOT NULL,
      nickname TEXT UNIQUE NOT NULL, salt TEXT NOT NULL, password_hash TEXT NOT NULL,
      cash NUMERIC NOT NULL DEFAULT 10000, holdings JSONB NOT NULL DEFAULT '{}'::jsonb,
      transactions JSONB NOT NULL DEFAULT '[]'::jsonb, banned_until BIGINT,
      ban_reason TEXT DEFAULT '', created_at BIGINT NOT NULL
    )`);
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS stocks (
      ...
      open_price NUMERIC NOT NULL, high NUMERIC NOT NULL, low NUMERIC NOT NULL,
      volume BIGINT NOT NULL DEFAULT 0, min_change NUMERIC NOT NULL DEFAULT 1,
      max_change NUMERIC NOT NULL DEFAULT 10, price_floor NUMERIC,
      floor_lock_minutes INTEGER NOT NULL DEFAULT 10, floor_lock_until BIGINT NOT NULL DEFAULT 0,
      floor_rise_remaining INTEGER NOT NULL DEFAULT 0, max_shares BIGINT NOT NULL DEFAULT 30,
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS price_history (
      id BIGSERIAL PRIMARY KEY, stock_id TEXT NOT NULL REFERENCES stocks(id) ON DELETE CASCADE,
      time BIGINT NOT NULL, price NUMERIC NOT NULL
    )`);
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS market_controls (
      ...
      direction TEXT NOT NULL DEFAULT 'normal', until_time BIGINT NOT NULL DEFAULT 0,
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS game_settings (
      id INTEGER PRIMARY KEY, starting_cash NUMERIC NOT NULL DEFAULT 10000,
      global_price_floor NUMERIC, floor_lock_minutes INTEGER NOT NULL DEFAULT 10,
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS notifications (
      ...
      message TEXT NOT NULL, type TEXT NOT NULL DEFAULT 'info', is_read BOOLEAN NOT NULL DEFAULT FALSE,
      created_at BIGINT NOT NULL
    )`);
    await client.query(`CREATE TABLE IF NOT EXISTS bank_transactions (
      ...
      receiver_id TEXT REFERENCES users(id) ON DELETE SET NULL, amount NUMERIC NOT NULL,
      memo TEXT DEFAULT '', type TEXT NOT NULL, created_at BIGINT NOT NULL
    )`);
    await client.query(`CREATE TABLE IF NOT EXISTS feedback (
      ...
      title TEXT NOT NULL, content TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'pending',
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS changes (
      ...
      feedback_id BIGINT, created_at BIGINT NOT NULL
    )`);
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS maintenance (
      id INTEGER PRIMARY KEY, enabled BOOLEAN NOT NULL DEFAULT FALSE,
      start_time BIGINT, end_time BIGINT, updated_at BIGINT NOT NULL
    )`);
    await client.query(`CREATE TABLE IF NOT EXISTS schema_migrations (
      version TEXT PRIMARY KEY, applied_at BIGINT NOT NULL
    )`);

    const legacyMigration = await client.query(`SELECT 1 FROM schema_migrations WHERE version=$1 LIMIT 1`, ["remove-hardcoded-stocks-v1"]);
    if (!legacyMigration.rows.length) {
      await client.query(`DELETE FROM stocks WHERE id IN ('SKNX','SAMS','TWAI','NVR','NFLX','PASC','LG','HYUN','NVDO','MHD')`);
      await client.query(`INSERT INTO schema_migrations(version, applied_at) VALUES($1,$2)`, ["remove-hardcoded-stocks-v1", Date.now()]);
    }

    const floorMigration = await client.query(`SELECT 1 FROM schema_migrations WHERE version=$1 LIMIT 1`, ["remove-default-stock-floors-v2"]);
    if (!floorMigration.rows.length) {
      ...
      await client.query(`INSERT INTO schema_migrations(version, applied_at) VALUES($1,$2)`, ["remove-default-stock-floors-v2", Date.now()]);
    }
    ...
    for (const stock of stockRows) {
      let held = 0;
      for (const user of userRows) held += Math.max(0, Math.floor(Number((user.holdings || {})[stock.id] || 0)));
      await client.query(`UPDATE stocks SET available_shares=GREATEST(0,max_shares-$2::BIGINT) WHERE id=$1`, [stock.id, held]);
    }

    await client.query(`INSERT INTO game_settings(id,starting_cash,global_price_floor,floor_lock_minutes,updated_at)
      VALUES(1,10000,NULL,10,$1) ON CONFLICT(id) DO NOTHING`, [Date.now()]);
    // 전체 커트라인은 더 이상 사용하지 않습니다.
    await client.query(`UPDATE game_settings SET global_price_floor=NULL WHERE id=1`);

    await client.query(`INSERT INTO maintenance(id,enabled,start_time,end_time,updated_at)
      VALUES(1,FALSE,NULL,NULL,$1) ON CONFLICT(id) DO NOTHING`, [Date.now()]);
